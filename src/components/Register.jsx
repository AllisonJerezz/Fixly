...
export default function Register() {
  ...
  async function submit(e) {
    e.preventDefault();
    setErr("");
    if (!isValidEmail(form.email)) { setErr("Por favor, ingresa un email válido."); return; }
    try {
      setLoading(true);
      await register(form);
      setSent(true);
          ...
          <div className="rounded-3xl border border-white/15 bg-white/10 p-8 shadow-2xl backdrop-blur">
            ...
            <form onSubmit={submit} className="grid gap-4">
              <div>
                ...
                <input name="username" value={form.username} onChange={onChange} placeholder="tu_usuario" required className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 text-white placeholder:text-white/70 outline-none focus:ring-2 focus:ring-sky-300/40" />
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-indigo-100/70">Usa 8+ caracteres. Recomendado: mayúsculas, minúsculas y números.</p>
              </div>
