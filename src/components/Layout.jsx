...
import { isAuthed, lsGetRequests, readProfile } from "../api";
...
export default function Layout({ authed, onLogout, children }) {
  ...
  // Paginas con fondo oscuro propio (prefijos incluidos)
  const darkRoots = [
    "/",
    "/login",
    "/register",
  ...
  // Refresco de conteo de requests si cambian en otra pestaña
  ...
  return (
    <div className="min-h-screen text-slate-900">
          ...
          <div className="hidden items-center gap-1 lg:flex">
            ...
            {logged && isProvider && (
              <>
                ...
                <NavLink to="/services/new" className={navClass}>
                  Publicar servicio
                </NavLink>
              </>
            )}
        ...
        {open && (
          <div className="border-b border-slate-200 bg-white/95 px-4 pb-4 pt-2 shadow lg:hidden">
            <div className="grid gap-2">
              {logged && (
                <NavLink to="/home" className={navClass} onClick={() => setOpen(false)}>
                  Home
                </NavLink>
              )}
              {logged && isProvider && (
                <>
                  ...
                  <NavLink to="/services/new" className={navClass} onClick={() => setOpen(false)}>
                    Publicar servicio
                  </NavLink>
                </>
              )}
              ...
              {logged && isClient && requestsCount > 0 && (
                <NavLink to="/requests" className={navClass} onClick={() => setOpen(false)}>
                  Mis solicitudes
                </NavLink>
              )}
              {logged && isProvider && (
              ...
              {logged && !isClient && !isProvider && (
                <NavLink to="/onboarding" className={navClass} onClick={() => setOpen(false)}>
                  Onboarding
                </NavLink>
              )}
              {logged && (
      ...
      <footer className="relative mt-1 bg-white">
        {!isLanding && (
          <div className="absolute -top-1 left-0 w-full overflow-hidden leading-[0]" aria-hidden>
            <svg viewBox="0 0 1440 120" preserveAspectRatio="none" className="h-12 w-full">
              <path d="M0,64 C240,128 480,0 720,16 C960,32 1200,112 1440,64 L1440,0 L0,0 Z" fill="#ffffff" />
            </svg>
          </div>
        )}
        <div className="mx-auto max-w-6xl px-4 py-8 text-center text-sm text-slate-600">
          Hecho con amor <span className="align-middle" />
          <br />
          <a href="mailto:soporte@fixly.test" className="text-sky-700 hover:underline">
            soporte@fixly.test
          </a>{" "}
          o{" "}
          <a href="tel:+56987654321" className="text-sky-700 hover:underline">
            +56 9 8765 4321
          </a>
        </div>
      </footer>
