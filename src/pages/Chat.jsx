...
export default function Chat() {
  ...
  const mapName = (id) => (id === ownerId ? "Cliente" : id === winnerId ? "Proveedor" : id || "Usuario");
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto min-h-[calc(100vh-5rem)] max-w-3xl px-4 py-16 text-white">
          <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-8 text-center shadow-xl backdrop-blur">
            ...
            <p className="mt-2 text-indigo-200/90">Asegúrate de tener una oferta aceptada y estar autenticado.</p>
            <div className="mt-6">
              <Link to={`/requests/${requestId}`} className="rounded-xl bg-indigo-500 px-4 py-2.5 font-semibold text-white shadow hover:bg-indigo-400">
      ...
      <div className="mx-auto min-h-[calc(100vh-5rem)] max-w-4xl px-4 py-8 text-white">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <h1 className="truncate text-2xl font-extrabold text-white/95">Chat - {req.title}</h1>
            <p className="mt-1 text-sm text-indigo-200/80">
              Participantes: <strong>{mapName(ownerId)}</strong> y <strong>{mapName(winnerId)}</strong>
            </p>
          </div>
          <Link to={`/requests/${req.id}`} className="shrink-0 rounded-xl border border-white/30 px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/5">
            Volver a solicitud
          </Link>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.05] shadow-2xl backdrop-blur">
          <div ref={listRef} className="max-h-[60vh] overflow-y-auto px-4 py-5">
            {messages.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/20 p-8 text-center text-indigo-100/90">Empiecen la conversacion!</div>
            ) : (
              <ul className="grid gap-3">
                {messages.map((m) => {
                  ...
                  const avatar = (
                    <div className={["mt-4 grid h-8 w-8 place-items-center rounded-full text-xs font-bold", mine ? "bg-indigo-500/80 text-white" : "bg-white/20 text-white/90"].join(" ")} title={mapName(fromId)}>
                      {mapName(fromId).slice(0, 1)}
                    </div>
                  );
                  const textNorm = String(m.text || "").replace(/\\n/g, "\n").replace(/\r\n?/g, "\n");
                  if (!textNorm.trim()) return null;
                  return (
                    <li key={m.id} className={`flex items-end gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                      {!mine && avatar}
                      <div className={["max-w-[80%] rounded-2xl border px-3 py-2 text-sm shadow", mine ? "border-indigo-300/40 bg-indigo-500/20 text-white/95" : "border-white/10 bg-white/[0.06] text-white/95"].join(" ")}>
                        <div className="mb-0.5 text-[10px] opacity-70">{mapName(fromId)} - {fmtTime(m.ts)}</div>
                        <div className="whitespace-pre-wrap">{textNorm}</div>
                      </div>
                      {mine && avatar}
                    </li>
                  );
          ...
          <div className="flex items-center gap-2 border-t border-white/10 p-3">
            ...
            />
            <button onClick={send} className="shrink-0 rounded-xl bg-indigo-500 px-4 py-2.5 font-semibold text-white shadow hover:bg-indigo-400 disabled:opacity-60" disabled={!text.trim()}>
              Enviar
            </button>
          </div>
        </div>
